...
class ResourceLoader extends Emitter {
  ...
  isCompleted() {
    return this.resourcesInfo.every(
      (r) => r.status === 'loaded' || r.status === 'error',
    )
  }
  ...
  setFactorErrors(info) {
    // 未开始，pre包含info.key
    const rs = this.resourcesInfo.filter(
      (r) => !r.status && r.pre && r.pre.indexOf(info.key) >= 0,
    )
    if (rs.length < 0) {
      return
    }
    rs.forEach((r) => {
      console.warn(
        `mark ${r.key}(${r.url}) as error because it's pre failed to load`,
      )
      r.status = 'error'
    })
  }
  ...
  findCanLoadResource() {
    const info = this.resourcesInfo.find(
      (r) =>
        r.status == undefined &&
        (r.pre == undefined || this.isPreLoaded(r.pre)),
    )
    return info
  }
  ...
  fetchResources() {
    ...
    while (info) {
      ...
      if (cache) {
        ...
        if (isOlder) {
          console.warn(
            `${info.key} is cached, but is older version, cache:${cache.ver} request: ${info.ver}`,
          )
        } else {
