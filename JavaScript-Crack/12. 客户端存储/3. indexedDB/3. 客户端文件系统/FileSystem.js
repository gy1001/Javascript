;(function () {
  ...
  class FileError {
    constructor(
      { code = 999, message = '未知错误' } = { code: 999, message: '未知错误' },
    ) {
      this.code = code
  ...
  Entry.copyFrom = function (entry) {
    const en = entry.isFile
      ? new FileEntry(entry.name, entry.fullPath, entry.file)
  ...
  class FileEntry extends Entry {
    ...
    append(content) {
      return this.getBlob().then(
        function (blob) {
          return this.write(
            new Blob([blob, Utils.contentToBlob(content, blob.type)]),
          )
        }.bind(this),
      )
    }
  ...
  class IndexedDBProvider {
    ...
    static getInstance(dbVersion, dbName, storeName) {
      return IndexedDBStoreFactory.getInstance(
        dbVersion,
        dbName,
        storeName,
      ).then((db) => {
    ...
    write(entry, content, type = 'text/plain') {
      ...
      if (!file) {
        // 不存在创建
        file = new FSFile(
          entry.fullPath.split(DIR_SEPARATOR).pop(),
          data.size,
          type,
          new Date(),
          data,
    ...
    getEntry(entry, path, { create, exclusive = false }, getFile = true) {
      ...
      return this._toPromise('get', path).then((fe) => {
        if (create === true && exclusive === true && fe) {
          ...
          throw new FileError({
            message: getFile
              ? FILE_ERROR.FILE_EXISTED
              : FILE_ERROR.Directory_EXISTED,
          })
        } else if (create === true && !fe) {
          //创建 && 文件不存在
          const name = path.split(DIR_SEPARATOR).pop(),
            newEntry = getFile
              ? new FileEntry(name, path)
              : new DirectoryEntry(name, path),
          ...
          (fe && fe.isDirectory && getFile) ||
          (fe && fe.isFile && !getFile)
        ) {
    ...
    getParent(entry) {
      ...
      const parentFullPath = entry.fullPath.substring(
        0,
        entry.fullPath.lastIndexOf(DIR_SEPARATOR),
      )
      //上级目录为根目录的情况
      if (parentFullPath === '') {
        return this.root
        ...
        if (cursor) {
          ...
          if (val.fullPath !== DIR_SEPARATOR) {
            ...
            if (
              ...
              results.push(
                val.isFile
                  ? new FileEntry(val.name, val.fullPath, val.file)
                  : new DirectoryEntry(val.name, val.fullPath),
              )
            }
          }
          cursor['continue']()
    ...
    ensureDirectory(entry, path) {
      ...
      return promiseForEach(
        dirs,
        (dir, index) => {
          return entry.getDirectory(dirs.slice(0, index + 1).join('/'), {
            create: true,
          })
        },
        true,
  ...
  FileSystem.isSupported = () => {
    self.indexedDB_ =
      ...
      self.msIndexedDB
    self.IDBTransaction =
      self.IDBTransaction || self.webkitIDBTransaction || self.msIDBTransaction
    self.IDBKeyRange =
