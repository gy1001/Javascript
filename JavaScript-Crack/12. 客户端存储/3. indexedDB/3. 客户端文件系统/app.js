;(function () {
      ...
      _buildMenus(cmds) {
        ...
        return (
          ...
          cmds
            .map(
              (c) =>
                `<li data-cmd='${c.cmd}'><a href="javascript:void(0)" data-cmd='${c.cmd}'>${c.name}</a></li>`,
            )
            .join('') +
          '</ul>'
        )
    ...
    entryFactory = {
      ...
      buildEntry: function (fullPath, isFile) {
        let name = fullPath.split('/').pop(),
          id = new Date().getTime()
        return `<figure data-id='${id}' draggable="false" class="item ellipsis">
                    <img data-id='${id}' draggable="false" class="${
          isFile ? 'file' : 'folder'
        }" src="images/${
          isFile ? 'file' : 'folder'
        }.png" alt="${name}" data-fullPath='${fullPath}'></img>
  ...
  const displayPath = function (fullPath) {
    ...
    getCurrentPath = function () {
      return [..._pMenu.querySelectorAll('li>a')]
        .pop()
        .getAttribute('data-fullpath')
    },
    //获得文件或者文件夹当前最大下标
    getEntrySequence = function () {
      let names = [..._files.querySelectorAll('figure>p')].map((el) =>
          el.innerText.trim(),
        ),
        fileIndex = -1,
        folderIndex = -1,
      ...
      names.forEach((name) => {
        if (name === '新建文件夹' && folderIndex < 0) {
          ...
          folderIndex =
            folderIndex > Number.parseInt(m[1])
              ? folderIndex
              : Number.parseInt(m[1])
        } else if ((m = fileRegex.exec(name)) && m.length > 1) {
          fileIndex =
            fileIndex > Number.parseInt(m[1])
              ? fileIndex
              : Number.parseInt(m[1])
        }
      })
  ...
  function registerRightMenu(fs) {
    ...
    _files.oncontextmenu = function (event) {
      ...
      ;(targetEntryEl = event.target), (menuType = getMenuType(targetEntryEl))
    ...
    function getEntryFromContext() {
      if (
        targetEntryEl &&
        (menuType == rightMenuFactory.menuType.File ||
          menuType == rightMenuFactory.menuType.Folder)
      ) {
  ...
  function registerPathEvents(fs) {
    _pMenu.addEventListener('click', (ev) => {
      let el = ev.target,
        fullPath = el.getAttribute('data-fullPath')
      // 进入特定目录
          ...
          try {
            if (FileUtil.isFolder(file)) {
              ...
              if (!fileEntry) {
                ...
                var start = new Date()
                fileEntry = await currentDir.getFile(file.name, {
                  create: true,
                })
